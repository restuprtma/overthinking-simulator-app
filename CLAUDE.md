...
This is the **Minimals MUI starter (v7.6.1)** adapted into a multi-tenant finance product ("Jesuit Finance"). Stack: Vite 7 + React 19 + React Router 7 (data router) + MUI v7 + Emotion + React Hook Form/Zod + Axios. No Redux / no SWR / no React Query — state is local + context.
...
- Access token stored in `sessionStorage` as `jesuit.access_token`; refresh token in `localStorage` as `jesuit.refresh_token` (keys in [constant.ts](src/module/core/features/auth/context/jwt/constant.ts)).
