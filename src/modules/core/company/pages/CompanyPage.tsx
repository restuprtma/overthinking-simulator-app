import React, { useState, useEffect } from 'react';
import { Box, Button, Typography, Card, TextField } from '@venturo/react-ui';
import { IconPlus, IconFilter, IconFilterOff, IconSearch } from '@tabler/icons-react';
import { CompanyTable } from '../components/CompanyTable';
import { CompanyForm } from '../components/CompanyForm';
import { CompanyFilters } from '../components/CompanyFilters';
import { CompanyUserManagement } from '../components/CompanyUserManagement';
import { useTableCompany } from '../hooks/useTableCompany';
import { useBoolean, usePermission, useDebounce, useDialogState } from '@/shared/hooks';
import { PERMISSIONS } from '@/app/constants/permission';
import type { Company } from '@/app/api/core/company/type';

/**
 * CompanyPage Component
 * Main page for company management with CRUD operations and user management
 */
const CompanyPage: React.FC = () => {
  const {
    companies,
    meta,
    isLoading,
    isDeleting,
    error,
    search,
    isActive,
    handlePageChange,
    handlePageSizeChange,
    handleSearch,
    handleStatusFilter,
    handleDelete,
    handleReset,
  } = useTableCompany();

  // Permission checks
  const { hasPermission } = usePermission();
  const canEdit = hasPermission(PERMISSIONS.COMPANY_EDIT);
  const canDelete = hasPermission(PERMISSIONS.COMPANY_DELETE);
  const canManageUsers = hasPermission(PERMISSIONS.COMPANY_MANAGE_USERS);

  // Dialog states
  const companyDialog = useBoolean();
  const filterDrawer = useBoolean();
  const userManagementDialog = useBoolean();

  // Company dialog state management
  const { dialogState, openCreate, openEdit, reset: resetDialogState } = useDialogState<Company>();

  const [selectedCompany, setSelectedCompany] = useState<Company | null>(null);

  // Local search state with debounce for header search
  const [headerSearchValue, setHeaderSearchValue] = useState(search);
  const debouncedHeaderSearch = useDebounce(headerSearchValue, 500);

  // Sync local search with global search state
  useEffect(() => {
    setHeaderSearchValue(search);
  }, [search]);

  // Trigger search when debounced value changes
  useEffect(() => {
    if (debouncedHeaderSearch !== search) {
      handleSearch(debouncedHeaderSearch);
    }
  }, [debouncedHeaderSearch]);

  // Reset header search value when filter drawer opens
  useEffect(() => {
    if (filterDrawer.value) {
      setHeaderSearchValue('');
      handleSearch(''); // Clear search immediately when drawer opens
    }
  }, [filterDrawer.value]);

  // Handlers
  const handleCreateClick = () => {
    openCreate();
    companyDialog.setTrue();
  };

  const handleEditClick = (company: Company) => {
    openEdit(company);
    companyDialog.setTrue();
  };

  const handleManageUsersClick = (company: Company) => {
    setSelectedCompany(company);
    userManagementDialog.setTrue();
  };

  const handleDialogClose = () => {
    companyDialog.setFalse();
    // Reset state after dialog animation completes
    setTimeout(() => {
      resetDialogState();
    }, 300);
  };

  const handleUserManagementClose = () => {
    userManagementDialog.setFalse();
    setTimeout(() => {
      setSelectedCompany(null);
    }, 300);
  };

  const handleSuccess = () => {
    // Table will auto-refresh via React Query cache invalidation
    handleDialogClose();
  };

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 2, p: 3 }}>
      {/* Error Message */}
      {error && (
        <Card sx={{ p: 2, bgcolor: 'error.light' }}>
          <Typography color="error">
            {error.message || 'An error occurred while loading companies'}
          </Typography>
        </Card>
      )}

      <Card sx={{ flexShrink: 0 }}>
        <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
          <div className="flex flex-col">
            <Typography variant="h6">Company Management</Typography>
            <Typography
              variant="inherit"
              fontSize={12}
              color="textSecondary"
              sx={{ display: { xs: 'none', sm: 'block' } }}
            >
              Manage companies, their information, and user assignments
            </Typography>
          </div>
          <div className="flex flex-row items-center gap-2 flex-wrap">
            {/* Search TextField - with debounce, hidden when filter drawer is open */}
            {!filterDrawer.value && (
              <TextField
                placeholder="Search Companies"
                size="small"
                value={headerSearchValue}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setHeaderSearchValue(e.target.value)
                }
                slotProps={{
                  input: {
                    startAdornment: <IconSearch size={18} stroke={1.5} />,
                  },
                }}
                sx={{
                  minWidth: { xs: '100%', sm: 200, md: 250 },
                  flexGrow: { xs: 1, sm: 0 },
                }}
              />
            )}

            {/* Filter Button - berubah menjadi Reset Filters ketika drawer terbuka */}
            {!filterDrawer.value ? (
              <Button
                variant="outlined"
                color="grey"
                size="md"
                onClick={filterDrawer.setTrue}
                className="!min-w-0 !px-3"
              >
                <IconFilter size={16} />
              </Button>
            ) : (
              <Button
                variant="outlined"
                color="error"
                size="md"
                onClick={() => {
                  filterDrawer.setFalse();
                  handleReset();
                }}
                className="flex items-center gap-2"
              >
                <IconFilterOff size={16} />
                <span className="hidden sm:inline">Reset Filters</span>
              </Button>
            )}

            <Button
              variant="solid"
              color="primary"
              size="md"
              onClick={handleCreateClick}
              disabled={isLoading}
              className="flex items-center gap-2"
            >
              <IconPlus size={16} />
              <span className="hidden sm:inline">Add Company</span>
            </Button>
          </div>
        </div>

        {/* Collapsible Filters */}
        <div
          className={`transition-all duration-300 ease-in-out overflow-hidden ${
            filterDrawer.value ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
          }`}
        >
          <CompanyFilters
            search={search}
            isActive={isActive}
            onSearchChange={handleSearch}
            onStatusFilter={handleStatusFilter}
          />
        </div>
      </Card>

      <Card
        sx={{
          p: 0,
          flex: 1,
          overflow: 'hidden',
          display: 'flex',
          flexDirection: 'column',
          minHeight: 0,
        }}
      >
        {/* Companies Table with fixed header and scrollable body */}
        <CompanyTable
          companies={companies}
          isLoading={isLoading}
          isDeleting={isDeleting}
          meta={meta}
          onEdit={handleEditClick}
          onDelete={handleDelete}
          onManageUsers={handleManageUsersClick}
          onPageChange={handlePageChange}
          onLimitChange={handlePageSizeChange}
          canEdit={canEdit}
          canDelete={canDelete}
          canManageUsers={canManageUsers}
        />
      </Card>

      {/* Company Dialog (Create/Edit) */}
      <CompanyForm
        open={companyDialog.value}
        mode={dialogState.mode}
        company={dialogState.item}
        onClose={handleDialogClose}
        onSuccess={handleSuccess}
      />

      {/* Company User Management Dialog */}
      {selectedCompany && (
        <CompanyUserManagement
          open={userManagementDialog.value}
          companyId={selectedCompany.id}
          companyName={selectedCompany.name}
          onClose={handleUserManagementClose}
        />
      )}
    </Box>
  );
};

export default CompanyPage;
