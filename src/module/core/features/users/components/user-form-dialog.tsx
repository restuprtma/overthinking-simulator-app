...
function makeSchema(t: TFunction, isEditing: boolean) {
  return z.object({
    ...
    password: isEditing
      ? z.string()
      : z.string().min(8, { message: t('validation.passwordMin') }),
...
export function UserFormDialog({ open, mode, seed, onClose, onSaved }: Props) {
  ...
  const ownedCompanyIds = useMemo(() => {
    ...
    return new Set(
      initialValue.companies.filter((c) => c.is_owner).map((c) => c.company_id)
    );
  }, [initialValue]);
  ...
  useEffect(() => {
    ...
    const initialCompanies = new Set(
      initialValue?.companies?.map((c) => c.company_id) ?? []
    );
